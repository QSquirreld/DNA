...
class GeneticAlgorithm(Generic[C]):
    ...
    # Выбираем случайным образом num_paticipants и берем из них дые лучших
    ...
    def _reproduce_and_replace(self) -> None:
        ...
        # kпродолжаем, пока не заполним особями все новое поколение
        ...
        # если число енчетное, то один лишний, поэтому удаляем его
    ...
    # Выволнение генетического алгоритма для max_generations итераций
    # и возвращение лучшей из найденых особей
    def run(self) -> C:
        best: C = max(self._population, key=self._fitness_key)
        for generation in range(self._max_generations):
            # ранний выход, если первышен порог
